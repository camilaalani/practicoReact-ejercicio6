...
const SelectorDeColores = () => {
  ...
  const manejarGuardarColor = () => {
    if (!/^[a-zA-Z]+$/.test(nombreColor)) {
      setMensajeError(
        "Por favor, ingresa solo letras para el nombre del color."
      );
      return;
    }
    ...
    if (nombreColor.length < 2 || nombreColor.length > 15) {
      setMensajeError(
        "Por favor, ingresa un color con más de una letra y menos de 15 letras."
      );
      return;
    }
    ...
    localStorage.setItem(
      "coloresGuardados",
      JSON.stringify(nuevosColoresGuardados)
    );
  ...
  return (
    <Container className="mt-5">
      ...
      <section className="d-flex flex-wrap mt-3">
        <ListaColoresGuardados
          coloresGuardados={coloresGuardados}
          manejarBorrarColor={manejarBorrarColor}
        ></ListaColoresGuardados>
      </section>
    </Container>
  );
